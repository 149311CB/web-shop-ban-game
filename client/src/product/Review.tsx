...
export const verifyReviewPermission = async (
  ...
  const { data } = await axios.get(
    `/api/reviews/auth/verify?gameId=${gameId}`,
    {
      headers: {
...
const Review: React.FC<{ gameId: string }> = ({ gameId }) => {
  ...
  const submitReview = async () => {
    try {
      const route = userReview
        ? "/api/reviews/auth/update"
        : "/api/reviews/auth/create";
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data } = await axios.get(`/api/reviews/all?gameId=${gameId}`);
  ...
  useEffect(() => {
    ...
    const getReviewByUser = async () => {
      const { data } = await axios.get(
        `/api/reviews/auth/get?gameId=${gameId}`,
